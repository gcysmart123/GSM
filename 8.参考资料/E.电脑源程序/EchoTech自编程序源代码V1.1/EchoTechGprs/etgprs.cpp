    ...
    ui(new Ui::EtGprs)
{
    ...
    connect(&butStart,SIGNAL(clicked()),this,SLOT(startGprs()));
    connect(&butStart,SIGNAL(clicked()),this,SLOT(CloseGprs()));
    ...
    gridPcLayout->addWidget(&labPort,0,0,1,1);
    gridPcLayout->addWidget(&editPort,0,1,1,1);
    gridPcLayout->addWidget(&labPcIP,1,0,1,1);
    gridPcLayout->addWidget(&editPcIP,1,1,1,1);
    ...
    hLayout->addWidget(&butStart);
    hLayout->addWidget(&butClose);
...
void    EtGprs::startGprs()
{
    QString tempStr;
    gprsStr.clear();
    tempStr =   editPort.text();
    gprsPort = new Win_QextSerialPort(tempStr,QextSerialBase::Polling);
    gprsPort->open(QIODevice::ReadWrite);
    gprsPort->setBaudRate(BAUD9600);
...
void    EtGprs::timerEvent(QTimerEvent* event)
{
    ...
    if(event->timerId() == time1s)
    {
        ...
        if(gprsStr!=NULL)
        {
            listLog.addItem(gprsStr);
            if(pcSend   ==  true)
            {
                textEtMsg.setText(gprsStr);
                pcSend  =   false;
            }
        }
        if(gprsStep == 0)
...
void    EtGprs::initGprs()
{
    ...
    switch(gprsStep)
    {
        ...
        case    2:  gprsPort->write("AT+CGATT=1\r");        break;//附着和分离GPRS业务员
        case    10: gprsPort->write("AT+CSTT=\"CMNET\"\r"); break;//CMNET设置接入点
        case    11: gprsPort->write("AT+CIICR\r");          break;//启动连接
        case    12: gprsPort->write("AT+CIFSR\r");          break;//本机地址
        case    13:
        {
            getTcpIp();
